...
var CreditCard = (function () {
    ...
    CreditCard.validateDigit = function (value) {
        value = Utils_1.SanitizeToDigitsWithAssertion(value);
        if (!value)
            return false;
        var exploded = value.trim().split(""), sum = 0, digit = exploded.pop(), check;
    ...
    CreditCard.validateFlag = function (value) {
        value = Utils_1.SanitizeToDigitsWithAssertion(value);
        if (!value)
            return false;
        ...
        for (var i in keys) {
            ...
            if (card.pattern.test(value))
                return card.slug;
        }
        return false;
    };
    CreditCard.validateModulo = function (value) {
        value = Utils_1.SanitizeToDigitsWithAssertion(value);
        if (!value)
            return false;
        var exploded = value.trim().split(""), sum = 0;
        exploded = exploded.reverse();
        for (var n = 0; n < exploded.length; n++) {
            var _value = parseInt(exploded[n]);
